...
static uint32_t rosterColorHook(void* pane,uint32_t vertex) {
    ...
    auto elapsed=GetTickCount64()-start;if(elapsed>=450)return color;
    ...
    float t=static_cast<float>(elapsed)/450.f;t=t*t*(3.f-2.f*t);
    ++rosterFadeReads;
...
static void pumpPreviewReloads(){if(enabled){finishPreviewReload(0);finishPreviewReload(1);}
    ...
    if(start&&GetTickCount64()-start>=450&&rosterFadeAt.compare_exchange_strong(start,0))log("UI ROSTER fade color reads="+std::to_string(rosterFadeReads.load()));
}
static uintptr_t layoutHook(void* object,uint32_t hash,const char* animation,uint32_t extra) {
    ...
    if(roster.animation(hash,animation)) {
        const auto backAt=rosterBackAt.load();
        const bool back=backAt&&GetTickCount64()-backAt<600;
        const bool fade=!roster.hidden&&!back&&hash==Layer2Roster::portraits&&
            !strcmp(animation,"icon_on_p2")&&rosterInstanceOwner==object;
        rosterFadeAt=0;
        const bool nativeExit=roster.hidden&&roster.accessoriesPlayer==1&&
            rosterInstanceOwner==object&&!rosterInstances.empty();
        if(!roster.hidden) {
            rosterExitOwner=nullptr;
            if(rosterExitPose&&strcmp(animation,"icon_in")) {
                for(auto instance:rosterInstances)animationReset(object,Layer2Roster::portraits,"icon_out",instance);
                log("UI ROSTER restored native exit start pose");
            }
        ...
        if(nativeExit) {
            ...
            log("UI ROSTER native exit during P2 Accessories");
        } else (roster.hidden?layoutHideOriginal:layoutShowOriginal)(object,Layer2Roster::portraits);
        log(std::string("UI ROSTER ")+(roster.hidden?"HIDE ":fade?"FADE IN ":"SHOW INSTANT ")+animation+" instances="+std::to_string(rosterInstances.size()));
    }
    if(layoutLogs++<400||hash==Layer2Roster::portraits||!strncmp(animation,"detail_",7)||!strncmp(animation,"cos_",4))log("LAYER2 ANIMATION "+hex(hash)+" "+animation);
    if(hash==0xdd6728d2) {
