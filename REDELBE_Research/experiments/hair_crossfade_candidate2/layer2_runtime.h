...
#include "layer2_crossfade_runtime.h"
static uintptr_t requestHook(void* object,uint32_t player,uint32_t chara,uint32_t costume,uint32_t face,uint32_t hair,uint32_t color,int hairColor) {
    if(!startingHairFade)cancelHairFade(player);
    if(player<2) {
        std::lock_guard<std::recursive_mutex> guard(mutex);
...
static void* loadHook(void* object,void* context,uint8_t character,void* slot,uint8_t extra) {
    cancelHairFade(0);cancelHairFade(1);
    if(enabled&&slot) {
...
static void pumpPreviewReloads(){if(enabled){finishPreviewReload(0);finishPreviewReload(1);pumpHairFades();}}
static uintptr_t layoutHook(void* object,uint32_t hash,const char* animation,uint32_t extra) {
    if(animation&&(!strcmp(animation,"detail_out")||!strcmp(animation,"cos_out_p1")||!strcmp(animation,"cos_out_p2"))){cancelHairFade(0);cancelHairFade(1);}
...
static void cycle(int direction,const char* input) {
    ...
    if(hairPlayer>=0) {
        ...
        ReloadScope reload;
        beginHairFade(req);
        struct FadeScope { bool previous=startingHairFade;FadeScope(){startingHairFade=true;}~FadeScope(){startingHairFade=previous;} } fadeScope;
...
static void install() {
    ...
    std::vector<Hook> hooks={
        {gamecode::get("previewRender"),bytes("40 55 53 56 57 41 54 41 55 41 56 41 57 b8 f8 66 00 00"),(void*)previewRenderHook,(void**)&previewRenderOriginal},
