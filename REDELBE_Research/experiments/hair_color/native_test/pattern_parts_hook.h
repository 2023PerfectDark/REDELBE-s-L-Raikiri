...
static bool maybe(){
 ...
 if(outcome==1)tickets::awardWin();
 if(!userconfig::current.flag("VersusPatternParts","enabled"))return false;
 if(outcome){
...
static void install(){
 if(!userconfig::current.flag("VersusPatternParts","enabled")&&!userconfig::current.flag("Tickets","enabled")){log("PATTERN PARTS disabled by INI");return;}
