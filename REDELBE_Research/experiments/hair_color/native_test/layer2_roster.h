...
struct Layer2Roster {
    ...
    bool hidden=false;
    int accessoriesPlayer=-1;
    bool animation(uint32_t layout,const char* name) {
        if(!name)return false;
        bool before=hidden;
        if(layout==0xdd6728d2&&!strcmp(name,"detail_pos_1p"))accessoriesPlayer=0;
        if(layout==0xdd6728d2&&!strcmp(name,"detail_pos_2p"))accessoriesPlayer=1;
