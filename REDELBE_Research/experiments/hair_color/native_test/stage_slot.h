...
inline std::string slot(std::string_view texture) {
    for(auto code:{"S0801LOS_OBORO","S0802LOS_OBORO","S0899LOS_OBORO"})
        if(texture==code)return std::string(texture);
    ...
    bool oboro=false;
    if(tail.size()>6&&tail.substr(tail.size()-6)=="_OBORO"){oboro=true;tail.remove_suffix(6);}
    if(tail.empty()||tail.size()>2)return "Unknown";
    for(char c:tail){if(c<'0'||c>'9')return "Unknown";second=second*10+c-'0';}
    if(oboro&&first==8){if(second==1)return "S0801LOS_OBORO";if(second==2)return "S0802LOS_OBORO";if(second==99)return "S0899LOS_OBORO";}
