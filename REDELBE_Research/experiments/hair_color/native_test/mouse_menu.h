...
static std::map<unsigned,bool> centralSmall;
static int customDetailFocus=10;
static unsigned customSlotIndex=0;
...
static float rotateLastX=0,rotateAnchorX=0;
static POINT rotateAnchorClient{};
static std::atomic<uintptr_t> verticalOwner{0};
static std::atomic<float> verticalOffset{0};
static float rotateAnchorY=0;
...
static bool wardrobePreview(){return wardrobeOpen&&(screen==2||screen==5||screen==6||(screen==0&&(rowHash==0x8f2023d0||rowHash==0x7454be7b||rowHash==0x534d6e7c)));}
...
static int listWheelSteps=0;
static uint64_t wheelContext=0;
...
static void activateRows(void* owner,uint32_t hash,unsigned kind,int selection) {
 if(rowHash!=hash||screen!=kind){resetRotation();target=-1;confirm=false;pulse=0;cursorIn.clear();}
 if(hash==0xf403bcba&&kind==2)wardrobePortrait=selection;
...
static void animation(void* owner,uint32_t hash,const char* name,unsigned instance) {
 if(!name)return;
 if(hash==0x6fa9278c)wardrobeOpen=true;
 if(userconfig::current.flag("Debug","log_ui_events")&&(hash==0x5228be3c||hash==0xd6bfa658))
  log("MOUSE CENTRAL layout="+hex(hash)+" instance="+std::to_string(instance)+" animation="+name);
 ...
 auto customDetails=[&](int selected) {
  auto layout=reinterpret_cast<uintptr_t>(layoutObject(owner,0,0xf30929e8,0));
  const char* panes[]={"base_cos","base_arrange","base_hair","base_glasses","base_plate_shogo_1","base_plate_shogo_2"};
  families[0x1235949d].clear();
  for(unsigned i=0;i<6;++i) {
   auto pane=paneNamed(layout,panes[i]);
   if(pane)families[0x1235949d][10+i]={pane,pane,0,0,true};
  }
  customDetailFocus=selected;activateRows(owner,0x1235949d,6,selected);
 };
 if(hash==0xf30929e8) {
  if(strcmp(name,"out"))wardrobeOpen=true;
  if(!strcmp(name,"active_info"))customDetails(customDetailFocus);
  if(!strcmp(name,"select_cos"))customDetails(10);
  if(!strcmp(name,"select_arrange"))customDetails(11);
  if(!strcmp(name,"select_hair"))customDetails(12);
  if(!strcmp(name,"select_glass"))customDetails(13);
  if(!strcmp(name,"select_shogo"))customDetails(customDetailFocus==15?15:14);
 }
 if(userconfig::current.flag("Debug","log_ui_events")&&(hash==0x1235949d||hash==0xf30929e8))
  log("MOUSE CUSTOM SLOT layout="+hex(hash)+" instance="+std::to_string(instance)+" animation="+name);
 if(hash==0xf30929e8&&!strcmp(name,"out")) {
  const bool toPortraits=screen==5&&wardrobePortrait>=0;
  resetRotation();active=false;target=-1;confirm=false;pulse=0;
  // The wardrobe resumes its existing portrait cursor without icon_on/loop.
  // Restore the last real portrait focus, never the mychara favorite marker.
  if(toPortraits){
   for(auto& entry:families[0xf403bcba]){auto layout=reinterpret_cast<uintptr_t>(layoutObject(owner,0,0xf403bcba,entry.first));auto pane=rootPane(layout);if(pane)entry.second={pane,pane,0,0,true};}
   activateRows(owner,0xf403bcba,2,wardrobePortrait);nextPulse=GetTickCount64()+250;
   log("MOUSE wardrobe portrait restored="+std::to_string(wardrobePortrait));
  }
 }
 if(hash==0x1235949d) {
  if(instance<10&&(!strcmp(name,"cursor_slot_in")||!strcmp(name,"cursor_slot_loop")||!strcmp(name,"cursor_slot_sel")||!strcmp(name,"cursor_slot_sel_after")))customSlotIndex=instance;
  if(instance>=10&&instance<=15&&
     (!strcmp(name,"cursor_cos_in")||!strcmp(name,"cursor_cos_loop")||
      !strcmp(name,"cursor_hair_in")||!strcmp(name,"cursor_hair_loop")||
      !strcmp(name,"cursor_shogo_in")||!strcmp(name,"cursor_shogo_loop")||
      !strcmp(name,"cursor_shogo_vip_in")||!strcmp(name,"cursor_shogo_vip_loop"))) {
   customDetails(static_cast<int>(instance));return;
  }
  if(!strcmp(name,"cursor_slot_in")||!strcmp(name,"cursor_slot_loop")) {
   families[hash].clear();
   for(unsigned i=0;i<10;++i) {
    auto layout=reinterpret_cast<uintptr_t>(layoutObject(owner,0,hash,i));
    auto pane=paneNamed(layout,"icon_cos");
    if(pane)families[hash][i]={pane,pane,0,0,true};
   }
   activateRows(owner,hash,5,static_cast<int>(instance));
  }
  if(!strcmp(name,"cursor_slot_sel")) {
   if(rowHash==hash){active=false;target=-1;confirm=false;pulse=0;}
  }
  return;
 }
 if(hash==0xd6bfa658&&!strcmp(name,"menu_out")) {
  restoreFocus();active=false;target=-1;confirm=false;pulse=0;
 }
 if(hash==0x5228be3c) {
  wardrobeOpen=false;resetRotation();
  // DOA Central uses cursor_loop focus events, not text_on. Refresh
  // every panel when focus returns, including after closing a child screen.
  if(!strcmp(name,"menu_l_on")||!strcmp(name,"menu_m_on")) {
   centralSmall[instance]=!strcmp(name,"menu_m_on");
   auto layout=reinterpret_cast<uintptr_t>(layoutObject(owner,0,hash,instance));
   auto pane=paneNamed(layout,!strcmp(name,"menu_l_on")?"menu_l":"menu_m");
   families[hash][instance]={pane,pane,0,0,true};
  }
  if(!strcmp(name,"cursor_loop")) {
   for(const auto& panel:centralSmall) {
    unsigned i=panel.first;
    auto layout=reinterpret_cast<uintptr_t>(layoutObject(owner,0,hash,i));
    auto pane=paneNamed(layout,panel.second?"menu_m":"menu_l");
    if(pane)families[hash][i]={pane,pane,0,0,true};
   }
   activateRows(owner,hash,0,static_cast<int>(instance));
  }
  if(!strcmp(name,"text_sel_decide")||!strcmp(name,"cursor_select")) {
   active=false;target=-1;confirm=false;pulse=0;
  }
  return;
 }
...
static bool suppress(void* owner,uint32_t hash,const char* name,unsigned instance) {
 if(!name||screen!=0||rowHash==0x5228be3c||rowHash==0xe825f3e2||rowHash==0xa3b4d739||!active||target<0||static_cast<int>(instance)==target)return false;
...
static WORD poll() {
 animationpreviewtest::poll(wardrobePreview()&&!dialog&&!reward);
 ...
 if(animationpreviewtest::expanded){resetRotation();pulse=0;confirm=false;target=-1;return 0;}
 if(haircolor::expanded||haircolor::pointerOver()){resetRotation();if(re)haircolor::expanded=false;pulse=0;confirm=false;target=-1;return 0;}
 if((!active&&!startup&&!dialog&&!reward)||pid!=GetCurrentProcessId()){resetRotation();resetStageGesture();stageSteps=0;pulse=0;target=-1;dialogTarget=-1;confirm=false;restoreFocus();return 0;}
 if(updatenotice::inWindow&&re)return XINPUT_GAMEPAD_B;
 bool stageInput=active&&screen==3&&!dialog&&!reward;
 if(auto notice=updatenotice::input(active&&screen==0&&rowHash==0xf326b46d&&!dialog&&!reward))return notice;
 bool listWheelInput=active&&!dialog&&!reward&&(screen==6||
  (screen==0&&(rowHash==0x7454be7b||rowHash==0x8f2023d0||rowHash==0x534d6e7c)));
 uint64_t context=(stageInput||listWheelInput)?(static_cast<uint64_t>(screen+1)<<32)|rowHash:0;
 if(context!=wheelContext){resetStageGesture();wheelContext=context;}
 if(stageInput||listWheelInput) {
 ...
 if(point.x<0||point.y<0||point.x>=rect.right||point.y>=rect.bottom){resetRotation();resetStageGesture();stageSteps=0;target=-1;confirm=false;restoreFocus();return 0;}
 ...
 if(!wardrobePreview()){verticalOffset=0;verticalOwner=0;}
 if(!wardrobePreview()||dialog||reward)resetRotation();
 if(wardrobePreview()&&!dialog&&!reward){
  if(re)resetRotation();
  // Drag starts only over the model area, outside the lists and footer.
  if(le&&x>-180.f&&y>-420.f&&y<420.f){rotateHeld=true;rotateLastX=rotateAnchorX=x;rotateAnchorClient=point;rotateAnchorY=y;rotateButton=0;rotateUntil=0;target=-1;confirm=false;pulse=0;restoreFocus();}
  if(rotateHeld){
   if(left){float dy=y-rotateAnchorY;if(abs(dy)>=1.f)verticalOffset=std::max(-200.f,std::min(200.f,verticalOffset.load()+dy*.30f));}
   auto button=rotationStep(x,left,now);
   if(rotateHeld){
    // Relative movement around the original click permits endless rotation.
    // No global cursor clipping: release/focus loss always restores free motion.
    POINT anchor=rotateAnchorClient;
    if(!ClientToScreen(window,&anchor)||!SetCursorPos(anchor.x,anchor.y)){resetRotation();button=0;}
    else {rotateLastX=rotateAnchorX;lastPoint=rotateAnchorClient;}
   }
   target=-1;confirm=false;pulse=0;return button;
  }
 }
 ...
 // Back remains available during a model refresh, even if its menu rows are
 // temporarily absent from the layout cache.
 if(re){target=-1;confirm=false;pulse=XINPUT_GAMEPAD_B;pulseEnd=now+65;nextPulse=now+180;return pulse;}
 if(pulse==XINPUT_GAMEPAD_B&&now<pulseEnd)return pulse;
 ...
 for(const auto& item:rows) {
  ...
  if(!read(r.root+0x113,alpha)||((screen==0||screen==3||screen==5||screen==6)&&alpha<240)||!read(r.root+0x104,flags)||!(flags&0x20)||!read(r.root+0xe0,matrix)||!read(matrix,m))continue;
 ...
 if(listWheelInput) {
  int scroll=wheelSteps(static_cast<int>(InterlockedExchange(&stageWheel,0)));
  if(scroll){
   if((scroll>0&&listWheelSteps<0)||(scroll<0&&listWheelSteps>0))listWheelSteps=0;
   listWheelSteps=std::clamp(listWheelSteps+scroll,-8,8);
   restoreFocus();target=-1;confirm=false;
  }
  if(le||re||moved)listWheelSteps=0;
  if(listWheelSteps) {
   if(now<pulseEnd)return pulse;
   pulse=0;if(now<nextPulse)return 0;
   pulse=listWheelSteps>0?XINPUT_GAMEPAD_DPAD_DOWN:XINPUT_GAMEPAD_DPAD_UP;
   listWheelSteps+=listWheelSteps>0?-1:1;
   pulseEnd=now+32;nextPulse=now+90;return pulse;
  }
 }
...
static void pad(DWORD player,XINPUT_STATE* state,DWORD result) {
 ...
 if(native){resetRotation();resetStageGesture();target=-1;dialogTarget=-1;stageSteps=0;confirm=false;pulse=0;restoreFocus();}
 else buttons=poll();
 buttons=updatenotice::buttons(buttons);
...
static void keyboard(unsigned char* keys) {
 ...
 if(updatenotice::inWindow){
  if(keys[confirmKey]&0x80)buttons|=XINPUT_GAMEPAD_A;
  if(keys[backKey]&0x80)buttons|=XINPUT_GAMEPAD_B;
  buttons=updatenotice::buttons(buttons);keys[confirmKey]=keys[backKey]=0;
 }
