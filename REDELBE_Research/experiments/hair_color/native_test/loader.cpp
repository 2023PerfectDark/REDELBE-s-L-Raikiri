...
static std::wstring selectPath(const wchar_t* path, DWORD access, DWORD disposition, void* caller) {
    ...
    // The initial palette is generated from vanilla hair. An explicit Layer2
    // replacement keeps its own albedo until mod-aware recoloring is supported.
    std::wstring hairColorPath;
    {
...
static void WINAPI traceExit(UINT code) {
    ...
    void* frames[24]{};auto n=CaptureStackBackTrace(0,24,frames,nullptr);for(USHORT i=0;i<n;i++){MEMORY_BASIC_INFORMATION m{};VirtualQuery(frames[i],&m,sizeof(m));wchar_t module[MAX_PATH]{};GetModuleFileNameW(static_cast<HMODULE>(m.AllocationBase),module,MAX_PATH);log("EXIT FRAME "+std::to_string(i)+" "+utf8(module)+" rva="+std::to_string(reinterpret_cast<uintptr_t>(frames[i])-reinterpret_cast<uintptr_t>(m.AllocationBase)));}
    realExit(code);
}
...
static void start() {
    ...
    updatenotice::gameVersion=wide(hash);
    birthdays::load();
    ...
    l2::loadCatalog();
    if(GetPrivateProfileIntW(L"Diagnostics",L"resource_only",0,(gameRoot+L"REDELBE_LR\\startup_test.ini").c_str())) {
        log("DIAGNOSTIC resource-only mode: native feature hooks omitted");
        installImports();return;
    }
    // Install resource/startup imports before slower native feature setup. The
    // game may query modules on another thread during initialization.
    if (!installImports()) {log("DISABLED: import hook preflight failed");return;}
    wardrobevoice::install();
    l2::install();
    try {auratest::install(hash);}catch(const std::exception& e){log(std::string("AURA TEST disabled: ")+e.what());}
