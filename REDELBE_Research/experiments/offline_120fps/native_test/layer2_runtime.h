...
namespace l2 {
struct Mod { std::string name; std::wstring iniPath; uint32_t slot; std::map<std::wstring,std::wstring> files; };
...
static void pumpHairColor(){
    bool menu=mousemenu::active&&!mousemenu::dialog&&mousemenu::screen==6;
    if(!menu&&!haircolor::visible)return;
    unsigned slot=mousemenu::customSlotIndex;
    std::lock_guard<std::recursive_mutex> guard(mutex);
    auto req=requests[0];haircolor::sync(menu&&slot<20&&req.object,req.hair,req.chara,slot);
    if(haircolor::take()){
        ReloadScope reload;
