...
static void track(void* control,unsigned side=0){
 if(!enabled||!control||side>1)return;
 ...
 if(!weakControls[1-side]||InterlockedCompareExchange(reinterpret_cast<volatile LONG*>(static_cast<BYTE*>(weakControls[1-side])+8),0,0)<=0)++generation;
...
static bool active(unsigned& serial){std::lock_guard<std::mutex> guard(stateMutex);serial=generation;for(auto control:weakControls)if(control&&InterlockedCompareExchange(reinterpret_cast<volatile LONG*>(static_cast<BYTE*>(control)+8),0,0)>0)return true;return false;}
static float gameSe(){
...
static void install(){
 ...
 enabled=true;HANDLE thread=CreateThread(nullptr,0,worker,nullptr,0,nullptr);if(!thread){enabled=false;throw std::runtime_error("sound thread failed");}CloseHandle(thread);
