 ...
 while node!=head and node not in seen and len(seen)<4096:
  ...
  if u32(node+0x20)==0xf403bcba:
   ...
   while pn!=layout+0x48 and pn not in pseen and len(pseen)<128:
    ...
    panes.append(dict(address=hex(pane),vtable=hex(u64(pane)-base),data=read(pane,0x180).hex()))
    pn=u64(pn+8)
   entries.append(dict(node=hex(node),id=u32(node+0x24),layout=hex(layout),vtable=hex(u64(layout)-base),data=read(layout,0x380).hex(),panes=panes))
  node=successor(node)
 out=dict(base=hex(base),manager=hex(manager),entries=entries)
 Path('analysis/roster_objects.json').write_text(json.dumps(out,indent=2))
 print('roster instances',len(entries),'manager',hex(manager));print([{k:v for k,v in e.items() if k not in ('data','panes')} for e in entries[:2]])
