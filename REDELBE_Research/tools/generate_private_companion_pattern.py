...
data=(root/'analysis/isolation/resolver_3767200.bin').read_bytes()[0xc0:0x156]
...
(root/'experiments/hair_color/native_test/private_companion_pattern.h').write_text(
 '#pragma once\n// Read-only native capture; relocated calls/data addresses masked.\n'
 f'static const GamePattern privateCompanionPattern={{"privateCompanion","{esc(data)}","{esc(mask)}",{len(data)},0}};\n')
print('Generated companion resolver pattern:',len(data),'bytes')
