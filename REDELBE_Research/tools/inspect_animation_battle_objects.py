...
try:
 ...
 import re
 roots=[]
 for match in re.finditer(r'LAYER2 PRIVATE BATTLE TRACE player=(\d+) object=([0-9A-Fa-f]+)',log.read_text()):
  side=int(match[1]);roots=[x for x in roots if x[0]!=side];roots.append((side,int(match[2],16)))
 ...
 dest=Path(__file__).resolve().parents[1]/'experiments/animation_browser/battle_objects.json';dest.write_text(json.dumps(dict(pid=int(sys.argv[1]),base=hex(base),objects=out),indent=2))
 print(json.dumps([{k:v for k,v in x.items() if k!='data'} for x in out],indent=2))
