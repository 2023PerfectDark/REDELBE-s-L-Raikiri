...
try:
 ...
 queue=[(u64(u64(p+8)+0x18),0,'P'+str(side+1)+'Body') for side,p in roots];seen=set();out=[]
 while queue and len(out)<2000:
  ...
  for off in range(0,len(raw),8):
   ...
   if q%8==0 and 0x100000000<=q<0x700000000000 and q not in seen:
    qv=u64(q)
    if base<=qv<base+0xa000000 or depth<2:queue.append((q,depth+1,hex(p)+'+'+hex(off)))
 dest=Path(__file__).resolve().parents[1]/'experiments/animation_browser/match_model_graph.json';dest.write_text(json.dumps(dict(pid=int(sys.argv[1]),base=hex(base),objects=out),indent=2))
