...
$sourceDll = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\experiments\mouse_menu\build\dinput8.dll'))
...
$schemaPath = Join-Path $gameDir 'REDELBE_LR\settings.schema.json'
Copy-Item -LiteralPath $iniPath -Destination $checkpoint
Copy-Item -LiteralPath $schemaPath -Destination $checkpoint
...
if ($ini -notmatch '(?im)^\[VersusPatternParts\]') {
    $addition = Get-Content -LiteralPath (Join-Path $PSScriptRoot '..\experiments\mouse_menu\settings\pattern_parts.ini') -Raw
    [IO.File]::WriteAllText($iniPath, $ini + "`r`n" + $addition, [Text.UTF8Encoding]::new($false))
}
Copy-Item -LiteralPath (Join-Path $PSScriptRoot '..\experiments\mouse_menu\settings\settings.schema.json') -Destination $schemaPath -Force
