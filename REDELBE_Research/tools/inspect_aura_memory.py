...
try:
 ...
 ids={0x7fa35bed:'body',0x6360b6f0:'head',0xf56f07de:'limb'}
  ...
  for off in range(0,m.size,4*1024**2):
   ...
   for fid,label in ids.items():
    ...
    while (pos:=data.find(pattern,pos))>=0:
     if len(hits)<300:hits.append(dict(effect=label,address=hex(int(m.base)+off+pos),region=hex(int(m.base)),offset=pos,context=data[max(0,pos-64):pos+128].hex()))
     pos+=4
 ...
 Path('analysis/aura_live_scan.json').write_text(json.dumps(result,indent=2))
