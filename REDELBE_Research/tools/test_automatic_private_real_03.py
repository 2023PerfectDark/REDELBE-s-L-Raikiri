...
root=Path('REDELBE_Research');out=root/'packages/automatic_private_test_03';stage=root/'analysis/isolation/auto_inputs_03';stage.mkdir()
...
for i,(_,file,meta,project) in enumerate(candidates):
 ...
 if meta['slot']=='KOK_COS_001':recipes[i+1]=json.loads((root/'analysis/isolation/hanabi_portable_recipe.json').read_text())
